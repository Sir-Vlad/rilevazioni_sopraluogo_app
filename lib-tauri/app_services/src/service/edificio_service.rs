...
use app_error::{AppResult, ApplicationError, InfrastructureError};
...
impl RetrieveManyService<EdificioDTO> for EdificioService {
    async fn retrieve_many(
        ...
        let mut conn = db
            .get_connection()
            .await
            .map_err(|e| ApplicationError::Infrastructure(InfrastructureError::DatabaseError(e)))?;
...
impl RetrieveOneService<EdificioDTO, String> for EdificioService {
    async fn retrieve_one(db: State<'_, impl DatabaseManager + Send + Sync>, id: String) -> AppResult<EdificioDTO> {
        let mut conn = db
            .get_connection()
            .await
            .map_err(|e| ApplicationError::Infrastructure(InfrastructureError::DatabaseError(e)))?;
...
impl CreateService<EdificioDTO> for EdificioService {
    async fn create(db: State<'_, impl DatabaseManager + Send + Sync>, item: EdificioDTO) -> AppResult<EdificioDTO> {
        let mut conn = db
            .get_connection()
            .await
            .map_err(|e| ApplicationError::Infrastructure(InfrastructureError::DatabaseError(e)))?;

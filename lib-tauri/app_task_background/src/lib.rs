...
impl TaskInfo {
    ...
    #[inline]
    pub fn status(&self) -> StatusTask { self.status }

    pub fn set_status(&mut self, status: StatusTask) { self.status = status; }
}
...
impl BackgroundManager {
    ...
    pub async fn start(&mut self, app_handle: Arc<tauri::AppHandle>) -> ResultTask {
        if self
            .is_running
    ...
    pub async fn start_task(&mut self, task: Arc<RwLock<dyn BackgroundTask>>) {
    ...
    pub async fn pause_task(&mut self, task_name: &str) {
    ...
    pub async fn resume_task(&mut self, task_name: &str) {
    ...
    pub async fn done_task(&mut self, task_name: &str) {
        if let Some(task) = self.find_task_in_running(task_name).await {
            self.remove_task_from_lists(&task);
    ...
    pub async fn restart_task(&mut self, task_name: &str) {
